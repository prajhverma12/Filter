...
def download_csv():
    ...
    logging.info("Return value for Main Sheet : {}".format(returned_value))
    time.sleep(10)
...
def filterStocks(filename):
    try:
        ...
        logging.info("Removed unnecessary rows")
        ...
        if not file.checkfile("Stocks-" + date.dateToday() + ".csv"):
            ...
            logging.info("File not present {}, Converting Dataset into csv".format("Stocks-" + date.dateToday() + ".csv"))
        else:
            logging.info("File already present {}, Deleting the file".format("Stocks-" + date.dateToday() + ".csv"))
